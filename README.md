...
오프라인 번역을 위해 오픈소스 Vision 모델을 조사했으나, 세로쓰기 일본어에서 모두 한계:

| 모델 | 크기 | 일본어 OCR | 세로쓰기 | 비고 |
|------|:---:|:---:|:---:|------|
| **Qwen2.5-VL 7B** | 4.7GB | ★★★★ | △ | 직접 테스트 — 반복 출력, 인식률 낮음 |
| **MiniCPM-V 2.6** | 5.8GB | ★★★★ | △ | OCRBench 높지만 환각(hallucination) 多 |
| **LLaVA 1.6** | 7B | ★★★ | ✗ | 일본어 학습 데이터 부족, 성능 저조 |
| **Moondream 2** | 1.7B | ★★ | ✗ | 너무 작아서 일본어 미약 |
| **GLM-OCR** | ~6GB | ★★★ | △ | 문서 특화지만 일본어 세로쓰기 미검증 |

> 학술 연구에서도 확인: 기존 Vision LLM들은 **세로쓰기 일본어를 가로쓰기보다 훨씬 못 읽음**. 파인튜닝 없이는 실전 사용 어려움.
