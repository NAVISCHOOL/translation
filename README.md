...
전통적 OCR 파이프라인도 함께 조사:

| 도구 | 방식 | 일본어 세로쓰기 | 비고 |
|------|------|:---:|------|
| **Ollama-OCR** | Vision LLM 기반 OCR 프레임워크 | △ | 결국 위 Vision 모델에 의존, 동일한 한계 |
| **olmOCR2** | Allen AI OCR VLM (7B) | △ | 영어 문서 특화, 일본어 세로쓰기 미지원 |
| **Manga OCR** | 만화 일본어 특화 OCR | ★★★★ | 세로쓰기는 잘 읽지만 **OCR만** 됨 (번역 별도) |
| **Tesseract** | 전통 OCR 엔진 | ★★ | 세로쓰기 인식률 극히 낮음 |

> **핵심**: 전통 OCR 도구들은 "읽기"와 "번역"이 분리되어 2단계 파이프라인이 필요.
> Vision LLM은 "읽기+번역"을 한번에 하지만, 로컬 7B급은 세로쓰기에 약함.
> 결국 **안티그래비티가 직접 보고 번역하는 것이 가장 정확하고 심플**.
